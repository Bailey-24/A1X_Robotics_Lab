...
def _save_result(output_dir: Path, get_task_info, exec_error: str | None) -> None:
    ...
    if info.get("status"):
        feedback_parts.append(f"status={info['status']}")
    if info.get("vlm_response"):
        feedback_parts.append(f"verifier={str(info['vlm_response'])[:400]}")
...
def main(cfg: DictConfig) -> None:
    ...
    trusted_get_task_info = (
        namespace.get("get_task_info") if env_name.startswith("a1x-real") else None
    )
    ...
    _save_result(
        log_dir,
        trusted_get_task_info or namespace.get("get_task_info"),
        exec_error,
    )
