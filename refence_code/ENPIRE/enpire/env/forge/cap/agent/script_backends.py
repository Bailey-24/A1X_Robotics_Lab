...
The backend class is selected by the ``robot:`` Hydra config group:
    ...
    uv run python run_script.py robot=yam ...
...
class RealA1XBackend(ScriptRunnerBackend):
    """Non-interactive lifecycle for A1X script execution."""

    def __init__(
        self,
        env: Any,
        cfg: Any,
        go_home_on_exit: bool = False,
        **_: Any,
    ) -> None:
        self._env = env
        self._cfg = cfg
        self._go_home_on_exit = bool(go_home_on_exit)
    ...
    def post_exec(self, exec_error: str | None, namespace: dict) -> None:
        _ = exec_error
        try:
            if self._go_home_on_exit and namespace.get("go_home") is not None:
                namespace["go_home"]()
        except Exception as exc:
            print(f"[run_script] A1X go_home failed: {exc}")
        finally:
            try:
                self._env.stop()
            except Exception as exc:
                print(f"[run_script] A1X stop failed: {exc}")
...
def get_backend(env_name: str, cfg: Any, env: Any) -> ScriptRunnerBackend:
    ...
    if env_name.startswith("a1x-real"):
        return RealA1XBackend(env=env, cfg=cfg)
