...
def create_env(env_name: str, viewer: bool = False, **kwargs):
    ...
    Env names:
        ...
        "a1x-real" — real Galaxea A1X hardware through the local bridge
    ...
    elif env_type == "a1x-real":
        from enpire.env.forge.cap.env.real_a1x.env import RealA1XEnv

        return RealA1XEnv(**kwargs)

    else:
        raise ValueError(
            f"Unknown env: {env_name!r}. "
            f"Available: yam, yam-real, a1x-real"
        )
