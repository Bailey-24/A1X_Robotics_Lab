...
def yam_profile() -> RobotProfile:
    ...
    xml_path: Path | None = None
    try:
        from enpire.env.forge.robot.models.station.paths import get_station_urdf, get_station_xml
...
def a1x_profile() -> RobotProfile:
    """Galaxea A1X single-arm profile.

    A1X is exposed as ``right`` so existing single-arm CAP scripts can use the
    conventional side name without inventing a third arm identifier.
    """
    joint_limits_low = np.array(
        [-2.8798, 0.0, -3.3161, -1.5708, -1.5708, -2.8798],
        dtype=np.float64,
    )
    joint_limits_high = np.array(
        [2.8798, 3.1416, 0.0, 1.5708, 1.5708, 2.8798],
        dtype=np.float64,
    )
    arm = ArmProfile(
        name="right",
        ...
        home_gripper_pos=np.array([1.0], dtype=np.float64),
        gripper_min=0.0,
        gripper_max=1.0,
        interp_kp=np.zeros(7, dtype=np.float64),
        interp_kd=np.zeros(7, dtype=np.float64),
