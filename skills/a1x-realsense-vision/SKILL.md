...
description: 当用户询问看到什么、能看到什么、需要观察/分析当前摄像头画面时触发，通过 RealSense D405 相机实时拍摄，调用 qwen3.5-plus 云端 VLM 进行视觉分析
---
...
通过 Intel RealSense D405 相机实时拍摄彩色帧，调用 **qwen3.5-plus**（云端 VLM，通过 `A1X_VLM_API_KEY` 鉴权）对图像进行分析，返回场景描述、物体识别、环境细节等结果。
...
- Python 依赖：`pyrealsense2`、`opencv-python`、`numpy`、`openai`、`pyyaml`
- 已设置环境变量 `A1X_VLM_API_KEY`
...
positional arguments:
  prompt          分析提示词（默认：图像中有什么？请详细描述场景和物体。）

options:
  --model NAME    VLM 模型名（默认：qwen3.5-plus）
...
| `401 Unauthorized` | 确认已设置 `export A1X_VLM_API_KEY=...` |
| `model not found` | 确认 `--model` 指定的模型名正确 |
